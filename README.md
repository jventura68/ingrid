...
- 🎯 Entry points to execute your program using `python -m <ingrid>` or `$ ingrid` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/jventura68/ingrid/branch/main/graph/badge.svg?token=ingrid_token_here)](https://codecov.io/gh/jventura68/ingrid)
[![CI](https://github.com/jventura68/ingrid/actions/workflows/main.yml/badge.svg)](https://github.com/jventura68/ingrid/actions/workflows/main.yml)

Awesome ingrid created by jventura68

## Install

from source
```bash
git clone https://github.com/jventura68/ingrid ingrid
cd ingrid
make install
```
...
```bash
$ ingrid run --port 8080
```

or

```bash
python -m ingrid run --port 8080
```
...
```bash
❯ ingrid --help
Usage: ingrid [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ ingrid create-user --help
Usage: ingrid create-user [OPTIONS] USERNAME PASSWORD
...
```bash
ingrid create-user admin admin --superuser
```
...
Run with `ingrid run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `ingrid create-user`.
...
-----------------------------------------------------
ingrid/__init__.py              4      0   100%
ingrid/app.py                  16      1    94%
ingrid/cli.py                  21      0   100%
ingrid/config.py                5      0   100%
ingrid/db.py                   10      0   100%
ingrid/models/__init__.py       0      0   100%
ingrid/models/content.py       47      1    98%
ingrid/routes/__init__.py      11      0   100%
ingrid/routes/content.py       52     25    52%
ingrid/routes/security.py      15      1    93%
ingrid/routes/user.py          52     26    50%
ingrid/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from ingrid.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in ingrid/default.toml.

### As environment variables
```bash
export ingrid_KEY=value
export ingrid_KEY="@int 42"
export ingrid_KEY="@jinja {{ this.db.uri }}"
export ingrid_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
ingrid_ENV=production ingrid run
```
